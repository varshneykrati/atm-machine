...
public class BillPaymentsController {
	...
	// 5/
	@PostMapping("/dth/bill/{dthvendor}")
	public DTH payDthBill(@PathVariable("dthvendor") String vendorName,@RequestBody DthBill dthBill) throws BillPaymentsException {
		return this.billPaymentsService.payUserBill(vendorName,dthBill);
	}

	// 6/
	@PostMapping("/electricity/bill/{electricityvendor}")
	public ElectricityBill payElectricityBill(@PathVariable("electricityvendor")String vendorName,@RequestBody ElectricityBillDto electricitybill)
			throws BillPaymentsException {
